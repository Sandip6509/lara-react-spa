...
import { BreadcrumbItem, type Task, TaskProps } from '@/types';
...
function useTaskFilters(initialParams: Record<string, string | undefined>) {
...
export default function Task({ tasks, queryParams, success }: TaskProps) {
    ...
    const columns = useMemo(() => [
        ...
        { accessor: 'created_by', label: 'Created By', render: (item: Task) => item.created_by.name, sortable: true },
        {
            accessor: 'actions', label: 'Actions', render: (item: Task) => (
                <div className="flex space-x-2 gap-2" >
                    <Link href={route('tasks.edit', item.id)}><SquarePen className='text-green-700 hover:text-green-500 transition-colors duration-300' /></Link>
                    <DeleteModel routeName={route('tasks.destroy', item.id)} />
                </div >
            ), sortable: false
        },
    ], [getInitials]);
